...
void antMultiSportProcessing::readDeviceFileLine
(
    const char *line
)
{
    ...
    if ( nbWords > 1 )
    {
        ...
        else if ( deviceType == C_MULTI_SPORT_DEVICE_ID )
        {
            amString deviceName = words[ 1 ];
            if ( isMultiSportSensor( deviceName ) )
            {
                appendMultiSportSensor( deviceName );
            }
        }
    }
}
